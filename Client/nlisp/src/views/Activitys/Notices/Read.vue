<template>
  <div id="read">
    <div class="main-button" id="bulletin-button" v-on:click="pictureButton()">
      <p>동아리 활동</p>
    </div>
    <div class="main-button" id="mento-menti-button" v-on:click="noticeButton()">
      <p>공지사항</p>
    </div>
    <p id="bulletin-board-title">공지사항</p>
    <div id="bulletin-board-container">
      <p id="bulletin-board-sub-title">{{ boardTitle }}</p>
      <div id="bulletin-board-main-button" v-on:click="noticeButton()">
        <p>목록</p>
      </div>
...
export default {
  ...
  methods : {
    ...
    getNoticeBoardData() {
      var getNoticeBoardDataParameters = {
        params : {
      ...
      this.$axios.get(this.$store.state.host + this.$store.state.urls.noticeBoards.readDetail, getNoticeBoardDataParameters)
      .then(res => {
        if(res.status == 200) {
          this.boardTitle = res.data.noticeBoards[0].title;
          this.boardContent = res.data.noticeBoards[0].content;
          this.createTime = res.data.noticeBoards[0].createdAt.substring(0, 10) + ' ' + res.data.noticeBoards[0].createdAt.substring(11, 16);
          this.userImageURL = res.data.noticeBoards[0].userImageURL;
          this.boardUsername = res.data.noticeBoards[0].userName;

          var addHitConfigs = {
            noticeBoardId : res.data.noticeBoards[0].id
          };

          this.$axios.post(this.$store.state.host + this.$store.state.urls.noticeBoards.hit, addHitConfigs)
          .then(res => {
