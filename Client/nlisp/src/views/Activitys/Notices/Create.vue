<template>
  <div id="create">
    ...
    <div id="bulletin-board-container">
      <input type="text" v-model.lazy="noticeBoardTitle" placeholder="글 제목을 입력해 주세요"/>
      <textarea v-model.lazy="noticeBoardContent"></textarea>
    </div>
    <div id="bulletin-board-create-button" v-on:click="noticeBoardCreateButton()">
      <p>확 인</p>
    </div>
...
export default {
  ...
  methods : {
    ...
    noticeBoardCreateButton() {
      var noticeBoardCreateParams = {
        title : this.noticeBoardTitle,
        content : this.noticeBoardContent
      };

      var noticeBoardCreateConfigs = {
        headers : {
          token : this.$store.state.users.token
        }
      };

      this.$axios.post(this.$store.state.host + this.$store.state.urls.noticeBoards.create, noticeBoardCreateParams, noticeBoardCreateConfigs)
      .then(res => {
        if(res.status == 201) {
          ...
          alert('관리자가 아닌데 글을 업로드하려고 하면 안됩니다(어찌 들어왔누)');
        } else {
