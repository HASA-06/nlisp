<template>
  <div id="main">
    ...
    <div id="bulletin-board-create-button" v-on:click="mentoMentiBoardCreate()">
      <p>확 인</p>
    </div>
...
export default {
  ...
  methods : {
    ...
    mentoMentiBoardCreate() {
      let mentoMentiBoardCreateParameter = {
      ...
      let mentoMentiBoardCreateHeaders = {
        headers : {
          token : this.$store.state.users.token
        }
      };

      this.$axios.post(this.$store.state.host + this.$store.state.urls.mentoMentiBoards.create, mentoMentiBoardCreateParameter, mentoMentiBoardCreateHeaders)
      .then(res => {
      ...
      .catch(error => {
        if(error.response.status == 400 && error.response.data.title == 'Not authorized user') {
          alert('멘토 본인만이 글을 작성할 수 있습니다. 본인이 아니신경우 업데이트 후 이용해주세요 :)');
          this.$router.push({
            name : 'BoardsMentoMentisMain'
          });
        } else {
...
#bulletin-board-container input {
  ...
  background-color: #ffffff;
  margin: 1.5vw 0 1.171875vw 2.5vw;
